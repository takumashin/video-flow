  ...
  try {
    ...
    if (!kind) {
      return NextResponse.json({
        error: '请上传图片、视频或音频文件（JPG/PNG/WebP、MP4/WebM/MOV、MP3/WAV 等）',
      }, { status: 400 })
    }

    const maxSize = getMaxSizeForKind(kind)
    if (file.size > maxSize) {
      const limitLabel = kind === 'image' ? '10MB' : kind === 'video' ? '50MB' : '10MB'
      return NextResponse.json({ error: `文件大小不能超过 ${limitLabel}` }, { status: 400 })
    }

    const buffer = Buffer.from(await file.arrayBuffer())
    const mimeType = resolveUploadMimeType(file, kind)
    const { id, url } = await saveUpload(buffer, mimeType, workspaceId, userId, file.name, folderId, kind)
